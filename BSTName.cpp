...
User BSTName::search(TreeEntry x){
    TreePointer t=root;
    search(x, t);
...
void BSTName::remove(TreeEntry x){
    if(remove(x, root)){
...
// remocao de tupla
bool BSTName::remove(TreeEntry x, TreePointer &p){
    ...
    //Localizar o x
    if( x < p->entry.name ){ return remove(x, p->leftNode); } // vai para esquerda
    else if( x > p->entry.name ){ return remove(x, p->rightNode); } // vai para direita
